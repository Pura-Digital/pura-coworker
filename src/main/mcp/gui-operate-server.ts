...
// Get Aiden data directory for persistent storage
// Use platform-appropriate paths:
// - macOS: ~/Library/Application Support/aiden
// - Windows: %APPDATA%/aiden
const COLAV_DATA_DIR =
  PLATFORM === 'win32'
    ? path.join(process.env.APPDATA || path.join(os.homedir(), 'AppData', 'Roaming'), 'aiden')
    : path.join(os.homedir(), 'Library', 'Application Support', 'aiden');

// Directory for storing GUI operate files (screenshots, etc.)
const GUI_OPERATE_DIR = path.join(COLAV_DATA_DIR, 'gui_operate');
...
const GUI_APPS_DIR = path.join(COLAV_DATA_DIR, 'gui_apps');
...
function getResourcesDirCandidates(): string[] {
  ...
  const envResources = process.env.COLAV_RESOURCES_PATH;
...
async function resolveCliclickPath(): Promise<string | null> {
  ...
  const envOverride = process.env.COLAV_CLICLICK_PATH;
...
async function resolvePythonExec(): Promise<PythonExec | null> {
  ...
  // 1) Explicit override (useful for debugging)
  const envPython = process.env.COLAV_PYTHON_PATH;
  const envPythonHome = process.env.COLAV_PYTHON_HOME;
...
async function executeCliclick(command: string): Promise<{ stdout: string; stderr: string }> {
  ...
  try {
    ...
    if (/Accessibility privileges not enabled/i.test(result.stderr || '')) {
      const hint =
        '\n\nmacOS permissions:\n' +
        '- System Settings → Privacy & Security → Accessibility: allow Aiden\n' +
        '- If running from a terminal: allow Terminal/iTerm\n' +
        '- Restart Aiden after granting permission and try again\n';
      throw new Error(
    ...
    const hint =
      '\n\nmacOS permissions:\n' +
      '- System Settings → Privacy & Security → Accessibility: allow Aiden\n' +
      '- System Settings → Privacy & Security → Automation: allow Aiden to control "System Events"\n';
...
async function takeScreenshot(
  ...
  try {
    ...
    const hint =
      '\n\nmacOS permissions:\n' +
      '- System Settings → Privacy & Security → Screen Recording: allow Aiden\n' +
...
async function takeScreenshotForDisplay(
  ...
  const disableImageOutput = process.env.COLAV_DISABLE_IMAGE_TOOL_OUTPUT === '1';
...
async function analyzeScreenshotWithVision(
  ...
  try {
    ...
    const prompt = `Return the grounding coordinates for ${elementDescription}.

**Note**: The image may contain yellow circle markers from previous clicks (for relative position reference only; they are not necessarily correct click targets). Markers use "#index" and normalized "[y,x]" coordinates. Ignore these markers; locate the actual UI element.
