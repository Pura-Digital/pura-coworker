...
describe('App startup lazy loading', () => {
  it('keeps ChatView eager while deferring heavier panels', () => {
    const source = fs.readFileSync(appPath, 'utf8');

    expect(source).toContain("import { ChatView } from './components/ChatView';");
    expect(source).not.toContain("const ChatView = lazy(() =>");
    ...
    expect(source).toContain('ChatSessionGate');
  });
